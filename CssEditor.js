...
class CSSEditor {
    ...
    buildSections() {
        ...
        for (let section of this.propertySections) {
            ...
            sectionDiv.className = 'tester_css_section';
            sectionDiv.dataset.sectionName = section.name.toLowerCase();

            let header = document.createElement('div');
            header.className = 'tester_css_section_header collapsed';
            header.textContent = section.name;
            header.addEventListener('click', () => this.toggleSection(header));
            sectionDiv.appendChild(header);

            let content = document.createElement('div');
            content.className = 'tester_css_section_content collapsed';
    ...
    createPropertyRow(propDef) {
        ...
        row.className = 'tester_css_property_row';
        let label = document.createElement('label');
        label.className = 'tester_css_property_row_label';
        label.textContent = propDef.label;
        row.appendChild(label);

        let valueContainer = document.createElement('div');
        valueContainer.className = 'tester_value_with_unit';

        let input;
        if (propDef.type === 'select') {
            input = document.createElement('select');
            input.className = 'tester_css_property_row_select';
            ...
            input.className = 'tester_css_property_row_input';
        }

        if (propDef.units) {
            valueContainer.appendChild(input);
            let unitSelect = document.createElement('select');
            unitSelect.className = 'tester_unit_select';
    ...
    filterSections(searchText) {
        ...
        for (let section of this.sectionElements) {
            ...
            if (!match) {
                const rows = section.querySelectorAll('.tester_css_property_row');
                for (let row of rows) {
